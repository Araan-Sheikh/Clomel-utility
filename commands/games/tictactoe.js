...
module.exports = {
    ...
    run: async (bot, message, args, ops) => {
        ...
        if (opponent.user.bot) return message.channel.send('DUDE, WHY DONT YOU UNDERSTAND BOTS ARE DUMB');
        if (opponent.user.id === message.author.id) return message.channel.send('why would you even play with yourself loner');
        ...
        try {
            ...
            if (!verification) {
                ...
                return message.channel.send(`Looks Like ${opponent} Doesnt Wants To Play... what a nerd`);
            }
            ...
            while (!winner && taken.length < 9) {
                ...
                await message.channel.send(stripIndents`
					**${user}, Pick a side or type \`end\` To forfeit.... like a loser would**
					\`\`\`
            ...
            ops.games.delete(message.channel.id);
            if (winner === 'time') return message.channel.send('Game has been ended due to inactivity');
            return message.channel.send(winner ? `**GG, ${winner}!**` : 'The Game Ended With a Draw');
        } catch (err) {
            ops.games.delete(message.channel.id);
            throw err;
        }
