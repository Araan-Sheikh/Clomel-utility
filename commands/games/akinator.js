...
module.exports = {
	...
	run: async (bot, message, args, ops) => {
		...
		try {
			...
			if (win === 'time') return message.channel.send('I won! i am smart innit');
			if (win) return message.channel.send('DAMMIT! IG you won this time');
			return message.channel.send('yesss! i guessed it right again! you are cool');
		} catch (err) {
			ops.games.delete(message.channel.id);
			return message.channel.send(`Akinator API is down! do not blame araan please thank you`);
		};
	}
};
